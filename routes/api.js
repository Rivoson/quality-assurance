...
module.exports = (app) => {
  app.get("/api/issues/:project", (req, res, next) => {
    ...
    Project.findOne({ project }, (err, projectName) => {
      if (err) next(err);
      if (!projectName)
        return res.status(400).send(`Unknown Project: ${project}`);
    ...
    Project.findOne({ project }, (err, projectName) => {
      ...
      if (projectName) {
        ...
        projectName.save((err, updatedProject) => {
          ...
          return res
            .status(200)
            .json(updatedProject.issues[updatedProject.issues.length - 1]);
        });
      } else {
        ...
        newProject.save((err, createdProject) => {
          ...
          return res
            .status(200)
            .json(createdProject.issues[createdProject.issues.length - 1]);
        });
      }
    });
  });
  ...
  app.put("/api/issues/:project", (req, res, next) => {
    ...
    Project.findOne({ project }, (err, projectName) => {
      ...
      if (!projectName)
        return res.status(400).send(`Unknown Project: ${project}`);
      var issueIndex = projectName.issues.findIndex(
        (issue) => issue._id === _id
      );
  ...
  app.delete("/api/issues/:project", (req, res, next) => {
    ...
    Project.findOne({ project }, async (err, projectName) => {
      ...
      var issueIndex = projectName.issues.findIndex(
        (issueToRemove) => issueToRemove._id === _id
      );
