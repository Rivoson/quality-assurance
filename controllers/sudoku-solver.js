class SudokuSolver {
  validate(puzzleString) {
    ...
    if (puzzleString.length == 0 || puzzleString == undefined) {
      ...
      //we check the length and the presence of letters
  ...
  checkRowPlacement(puzzleString, row, column, value) {
    ...
    if (rowArray.indexOf(value) == -1 && rowArray[column - 1] == ".") {
      ...
      //we already have that value there
      return true;
    } else return false;
  }
  ...
  solve(puzzleString) {
    ...
    if (this.checkBoard(puzzleString)) {
      var order = this.compileOrder(puzzleString); //this are the missing cells
      var i = 0; //counter to navigate the missing cells
      var val = 1; //the initial value we will put in the emty cell
